...
where $\mathbf{s}_i$ references the $i^{th}$ spatial location and $\mathbf{t}_j$ references the $j^{th}$ time point. Then, we assume the errors 

\begin{equation}
\bm{\epsilon} \sim \text{N}(\mathbf{0}, \bm{\Sigma}),
\end{equation}

where $\bm{\Sigma}_{i, j} = \sigma^2 \rho_1 (\vert\vert \mathbf{s}_i - \mathbf{s}_{i'} \vert \vert) \rho_2 (\vert\vert \mathbf{t}_j - \mathbf{t}_{j'} \vert \vert)$.
...
\end{pmatrix} = 
\begin{pmatrix}
\bm{\Sigma}_{s, scurr} & \bm{\Sigma}_{s, ucurr} \\
\mathbf{X}_{scurr}' & \mathbf{X}_{ucurr}'
\end{pmatrix} 
\begin{pmatrix}
\mathbf{b}_{scurr} \\
\mathbf{b}_{ucurr}
...
where $s$ denotes the sampled sites (past and present), $scurr$ denotes the current sampled sites, and $ucurr$ denotes the current unsampled sites. $\bm{\lambda}$ is an $n$ by 1 vector, where $n$ denotes the number of sampled sites (both past and present). 

Then, we can solve for the prediction weights as

$$
\bm{\lambda}_s = \mathbf{b}_{sall}' + \mathbf{b}_{ucurr}' (\bm{\Sigma}_{ucurr, s}\bm{\Sigma}_{ss}^{-1}) - \mathbf{b}'_{ucurr}(\bm{\Sigma}_{ucurr, s} \bm{\Sigma}_{ss}^{-1})\mathbf{X}_s(\mathbf{X}_s'\bm{\Sigma}_{ss}^{-1}\mathbf{X}_s)^{-1}\mathbf{X}_s'\bm{\Sigma}_{ss}^{-1} + \mathbf{X}_{ucurr}(\mathbf{X}_s'\bm{\Sigma}_{ss}^{-1}\mathbf{X}_s)^{-1}\mathbf{X}_s \bm{\Sigma}_{ss}^{-1}
$$

where $\mathbf{b}_{sall}$ is a vector of 1's and 0's, with 1's for the sampled sites in the current year and 0's for the sampled sites in the past.
...
which has a prediction variance that can be found by solving
