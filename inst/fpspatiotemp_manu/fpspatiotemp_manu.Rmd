...
The Taylor Corridor in the east-central region of Alaska is a popular habitat for moose and other wildlife. Abundance surveys for moose are performed in the Taylor Corridor of the Tok region of Alaska annually (Figure \ref{fig:tokplot}) so that biologists have an idea about the abundance of moose each year. In particular, surveys were conducted every year from 2014 through 2020 in every year except 2016, during which there was not sufficient snow cover to perform a survey. The spatial sampling frame for one particular survey consists of `r nspat` sites. There are a total of `r ntime` unique time points represented in the data, including the missing year of 2016. Therefore, $N$ is `r nspat * ntime`. 
...
```{r, fig.cap = "\\label{fig:sitepredmap} A map of the predictions for the sites in the year 2020 from the spatio-temporal model. A site with a grey dot in the center means that the site was sampled in 2020."}
...
pred_df <- tibble(year = c(2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021),
                  ...
                  method = "ST-FPBK")
ggplot(data = pred_df, aes(x = year, y = prediction)) +
...
fit_obj_14 <- slmfit(formula = totalmoosena ~ newstrat,
                       data = moose_final_all |> dplyr::filter(Surveyyear == 2014),
                       xcoordcol = "xcoords", ycoordcol = "ycoords")
fit_obj_15 <- slmfit(formula = totalmoosena ~ newstrat,
                       data = moose_final_all |> dplyr::filter(Surveyyear == 2015),
                       xcoordcol = "xcoords", ycoordcol = "ycoords")
fit_obj_17 <- slmfit(formula = totalmoosena ~ newstrat,
                       data = moose_final_all |> dplyr::filter(Surveyyear == 2017),
                       xcoordcol = "xcoords", ycoordcol = "ycoords")
fit_obj_18 <- slmfit(formula = totalmoosena ~ newstrat,
                       data = moose_final_all |> dplyr::filter(Surveyyear == 2018),
                       xcoordcol = "xcoords", ycoordcol = "ycoords")
fit_obj_19 <- slmfit(formula = totalmoosena ~ newstrat,
                       data = moose_final_all |> dplyr::filter(Surveyyear == 2019),
                       xcoordcol = "xcoords", ycoordcol = "ycoords")
fit_obj_20 <- slmfit(formula = totalmoosena ~ newstrat,
...
ggplot(data = pred_plot_df, aes(x = year, y = prediction,
  ...
  #  facet_grid(~method, scales = "free", switch = "x", space = "free_x") + 
  #   theme(strip.placement = "outside")

ggplot(data = pred_plot_df, aes(x = year, y = prediction,
                                colour = method, group = method)) +
  geom_errorbar(aes(ymin = prediction - 1.645 * se,
                    ymax = prediction + 1.645 * se),
               position = "dodge2") +
  theme_minimal() +
