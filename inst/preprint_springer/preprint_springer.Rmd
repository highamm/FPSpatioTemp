...
@conn2015using  perils: bottom-right of page 239 and bottom-left of page 237

MENTION EXPERT KNOWLEDGE IN BAYESIAN APPROACH: IMPROPER UNIFORM PRIOR ON COVARIANCE PARAMETER DOES NOT TRANSLATE TO LOG SCALE, FOR EXAMPLE.
Ver Hoef upper bound on harbor seal abundance blow up.


Clarification on Finite correction
N = 10, n = 8, generated iid normal with unknown mean $\mu$ and known variance $\sigma^2 = 4$. If we care about the mean generating the process, then we use a standard error of sqrt(4) / 8 and our point estimate is the sample mean. If we care about the realized mean (as we do in most monitoring scenarios), then we use a standard error with a correction factor but our point estimate is still the sample mean.
...
Though there are a few types of models for the errors that can be built from equation \ref{equation:basicmod} by setting certain error variances to 0 (e.g. a sum-with-error model sets $\sigma^2_{\omega} = 0$ while a separable model sets all covariance parameters equal to $0$ except $\sigma^2_{\omega}$ and uses the structure for $\mathbf{R}_{st}$ given in equation \ref{equation:Rst} below) and/or by allowing $\mathbf{R}_{st}$ to take certain forms, we focus only on the product-sum model [@de2001product; @de2001space]. In a common formulation of the product-sum model, $\mathbf{R}_{st}$ is
\mbox{}
\begin{equation}
\label{equation:Rst}
