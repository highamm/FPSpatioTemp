...
soil samples, etc, through time. A common goal of ecological monitoring
...
data is collected everywhere within a subset of these areal sampling
units. The remaining areal units are then left unsampled. We refer to
the goal of predicting abundance in the most recent time point as
...
and the sum model. Chen, Genton, and Sun (2021) provide a review for
...
hierarchical model allows for a large amount of flexibility in the model
...
Also, the product-sum model is non-separable, giving it considerably
more flexibility than a separable model for the covariance (S. De Iaco,
Palma, and Posa 2015; Dumelle et al. 2021). Xu and Shu (2015) claim that
the product-sum model is the most widely used spatio-temporal model used
...
total response across all sites and all time points. More typical would
be the case where \(\mathbf{b}_a\) has \(1\)'s for all sites in the
current time point, and all other elements are \(0\), in which case we
would be predicting the total response in the current time point.
...
during optimization can vary from application to application. We the
